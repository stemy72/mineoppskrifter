/*
  # Fix Shared Recipes View

  1. New View
    - `accessible_recipes_view` - A view that joins recipes with related data and filters by user access
  
  2. New Functions
    - `get_accessible_recipes(user_email text)` - Returns all recipes a user has access to
    - `get_recipe_with_details(recipe_id uuid, user_email text)` - Returns a single recipe with all details
  
  3. Security
    - Functions are SECURITY DEFINER to ensure proper access control
    - RLS policies remain in place as a secondary security layer
*/
...
GRANT EXECUTE ON FUNCTION get_recipe_with_details TO authenticated;
